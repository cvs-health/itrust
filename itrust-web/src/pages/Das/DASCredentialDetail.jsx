...
export default function DASCredentialDetail( {das, setStatus}) {
    ...
    useEffect(() => {
        ...
        const fetchCredentialMetadata = async () => {
            ...
            try {
                const data = {
                    entityDigitalAddress: das?.organization?.digitalAddress,
                    entityDid: das?.organization?.did,
                }
    ...
    const handleRevoke = async (c) => {
        ...
        const response = await revokeCredential(das?.organization.did, c.credentialId);
        if (response) {
