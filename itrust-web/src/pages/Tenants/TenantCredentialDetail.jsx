...
import { BrandingWatermark, ExpandCircleDown, PersonRemove, Report } from '@mui/icons-material';
...
export default function TenantCredentialDetail( {tenant, setStatus}) {
    ...
    useEffect(() => {
        ...
        const fetchCredentialMetadata = async () => {
            ...
            try {
                const data = {
                    entityDigitalAddress: tenant?.organization?.digitalAddress,
                    entityDid: tenant?.organization?.did,
                }
    ...
    const handleRevoke = async (c) => {
        ...
        const response = await revokeCredential(tenant?.organization.did, c.credentialId);
        if (response) {
    ...
    return loading ? (
        ...
        <Box display="flex" flexDirection="row" flexWrap="wrap" gap={1}>
            {credentials && credentials.map((credential) => (
                <Card elevation={4} key={credential.ID} sx={{ minWidth: "24%" }}>
                    <CardHeader
                        ...
                        action={
                            ...
                            {credential.revoked && (
                                <Button startIcon={<Report />} color="secondary">
                                Revoked
                            </Button>
                            )}
                            </>
                    ...
                    <CardContent>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" gap={1}>
                            {/* <QRCode value={JSON.stringify(credential)} size={128} /> */}
                            <Box display="flex" flexDirection="column">
                                <Typography variant="subtitle1">Name</Typography>
                                <Typography variant="body1" gutterBottom>{credential.entityName}</Typography>
                                <Typography variant="subtitle1">Issued On</Typography>
                                <Typography variant="body1" gutterBottom>{format(parseISO(credential.CreatedAt), DATE_FORMAT)}</Typography>
                                <Typography variant="subtitle1">Type</Typography>
                                <Typography variant="body1" gutterBottom>{credential.credentialSchema.credentialTypes.map((ct) => ct.name).join(", ")}</Typography>

                                <Button endIcon={<ExpandCircleDown color="primary" />} onClick={() => handleShowDetails(credential)} color="primary">
